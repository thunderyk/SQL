...
지역이름(region_name), 나라이름(country_name)으로
...
select em.employee_id,
       ...
       em.hire_date,
       jh.end_date
from job_history jh, jobs jo, employees em
where jh.employee_id = em.employee_id and
      jh.job_id = jo.job_id and jo.job_id ='AC_ACCOUNT';
...
select de.department_id,
       ...
       co.country_name,
       re.region_name
from departments de, employees man, locations lo, countries co, regions re
where de.manager_id = man.employee_id
