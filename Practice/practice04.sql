...
select employee_id,
       ...
       em2.avg_salary,
       em2.max_salary
from employees em, (select avg(salary) avg_salary, max(salary) max_salary from employees) em2
where em.salary >= em2.avg_salary and
em.salary <= em2.max_salary
order by salary asc;

/*문제3.
직원중 Steven(first_name) king(last_name)이
소속된 부서(departments)가 있는 곳의 주소를 알아보려고 한다.
도시아이디(location_id), 거리명(street_address), 
우편번호(postal_code), 도시명(city), 주(state_province), 나라아이디(country_id) 를 출력하세요
(1건) 
*/--문제에서 king k가 소문자인데 대문자로 해야 나옴
...
select em.employee_id,
       ...
       em.salary,
       em.department_id
from employees em, (select department_id,  max(salary) salary from employees group by department_id) s
where em.department_id = s.department_id and em.salary = s.salary
order by salary desc;
...
select em.salary,
       jo.job_title
from (select sum(salary) salary, job_id from employees group by job_id) em, 
     (select job_title job_title, job_id from jobs) jo
